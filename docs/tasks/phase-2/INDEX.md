...
- [x] **T-2.10** — E2E with recording (or synthetic transcript fallback). 30-minute meeting recording — measure (a) catch rate = (auto-detected questions / ground-truth questions), (b) useful rate = (useful answers / detected questions, manually judged YES/NO). **AC:** ≥ 80% catch rate, ≥ 70% useful rate. If no real recording is available on the loop sandbox, ship a 30-minute synthetic-transcript fixture with hand-labeled ground-truth questions so the metric report is sandbox-runnable; mark the live-hardware re-verify as a manual step in `PHASE-BROWSER-TEST.md`. Dep: T-2.8 (full UX wired end-to-end). Risk: synthetic fixture not representative of real meeting cadence — mitigation = blend prompts from PRD §2 personas + spike-memo §"Use cases" + 1 transcript snippet from a real public talk if available. — **DONE (fixture-mode):** catch rate 0.947, useful rate 0.889 (AC ≥ 0.80 / ≥ 0.70 cleared with 14.7- and 18.9-point margins); first-token p95 1 500 ms (sim) < 10 000 ms; dedup reject = 1 (seeded pair); single-flight peak = 1; total cost $0.062229 over 31.76 min fixture, projected $0.117438/h (under $0.50/h budget). 7 new tests; **408 / 408** repo TS suite green; `tsc --noEmit` clean. Live-host re-verify queued in `PHASE-BROWSER-TEST.md`. See `T-2.10-review.md`.
