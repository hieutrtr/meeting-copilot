...
- [x] **T-W.3** — _Landed: 7 unit tests defined (5 INDEX-mandated + 2 sentinels — `which`-fallback explicit + spawn-failure typed); argv-array regression guard (Phase 4 R-2 reused) BLOCKING; zero new deps (`thiserror` already pinned for repo/auth/bridge — std-only otherwise); sync core API (live stdout/stderr streaming wrapper deferred to T-W.7's Tauri command); 3-arm probe ladder (`/opt/homebrew/bin/brew` → `/usr/local/bin/brew` → `which brew`); `Cargo.toml` UNCHANGED; cargo re-verify deferred to PHASE-MANUAL-VERIFY._  Auto-install helper: `crates/helper-daemon/src/setup_install.rs` (NEW) exposing `pub fn install_via_brew() -> Result<InstallReport, InstallError>` and `pub fn build_manual_install_url() -> &'static str`. Brew path: shell out to `which brew` → if `/opt/homebrew/bin/brew` (Apple Silicon) or `/usr/local/bin/brew` (Intel) found, run `brew install --cask blackhole-2ch` via `std::process::Command`. Stream stdout / stderr to a `tokio::sync::mpsc` channel for the UI. Manual path: returns `https://existential.audio/blackhole/` (the official .pkg landing page; coupon is permanent). **AC:** 5 unit tests using a `BrewProbe` trait seam: `brew_detected_apple_silicon_prefix`, `brew_detected_intel_prefix`, `brew_not_found_returns_manual_url`, `install_command_argv_array_form_no_shell` (assert `["brew", "install", "--cask", "blackhole-2ch"]` array passed to `Command`, not a shell string), `install_aborted_when_brew_exit_nonzero_surfaces_stderr`. **Dep:** T-W.2 (install caller polls detect to know when to stop). **Risk:** Homebrew prompts for sudo via `osascript` admin dialog; the dialog appears outside Tauri's window. Documented in T-W.1 design §"Permission UX" inventory; wizard step renders `"You may see a macOS password prompt — that's Homebrew installing the .pkg."`. **Files touched:** `crates/helper-daemon/src/setup_install.rs` (NEW), `crates/helper-daemon/src/lib.rs` (re-export), `crates/helper-daemon/Cargo.toml` (+`tokio` mpsc feature already present). **Diff size:** ~250–400 LOC.

- [x] **T-W.4** — _Landed: 9 unit tests defined (5 INDEX-mandated + 4 sentinels — rollback-on-failure, insufficient-sub-list, idempotent-rejects-non-stacked, UID matcher coverage); `is_stacked_key_is_1_for_multi_output_not_0` BLOCKING regression guard with multi-assertion defence in depth (literal value + constant inequality + constant value); idempotency rejects `is_stacked=false` aggregates with our brand UID (R-W.3 mitigation in idempotency code); `Cargo.toml` UNCHANGED (T-W.2 already added the macOS-gated FFI deps); `unsafe` FFI body in `RealAggregateDeviceCreator::create` deferred to T-W.7 (shares `cfStringForProperty` plumbing with `RealCoreAudioProbe::enumerate_*` per `blackhole.rs:153` deferred-FFI note); cargo re-verify deferred to PHASE-MANUAL-VERIFY._  CoreAudio Multi-Output configurator: `crates/audio-capture/src/multi_output.rs` (NEW) exposing `pub fn create_multi_output(name: &str, sub_device_uids: &[&str]) -> Result<AudioDeviceID, ConfigureError>`. Build a CFDictionary with keys `kAudioAggregateDeviceUIDKey="MeetingCopilotMultiOut"`, `kAudioAggregateDeviceNameKey=name`, `kAudioAggregateDeviceMasterSubDeviceKey=<built-in-output-uid>`, `kAudioAggregateDeviceSubDeviceListKey=[{uid: bh}, {uid: built-in}]`, `kAudioAggregateDeviceIsStackedKey=1` (1 = Multi-Output, 0 = Aggregate). Call `AudioHardwareCreateAggregateDevice(dict, &mut id)`. Idempotent: if a device with the same UID already exists, return its ID (no recreate). **AC:** 5 unit tests: `creates_multi_output_with_correct_keys` (mock `AudioHardwareCreateAggregateDevice` records the dict and asserts key set), `idempotent_returns_existing_device_id_when_uid_match`, `rejects_when_blackhole_uid_missing_from_sub_devices`, `is_stacked_key_is_1_for_multi_output_not_0` (regression guard — 0 creates a regular Aggregate which routes input not output), `master_sub_device_defaults_to_built_in_output`. **Dep:** T-W.2. **Risk:** Aggregate Device is *transient* — gone after reboot. Mitigation: T-W.7 (startup integration) re-runs detect → recreate if missing. Documented in T-W.8 SETUP.md "FAQ" section. Plus: `kAudioAggregateDeviceIsStackedKey` literal `1` vs `0` is the difference between Multi-Output and Aggregate; mistake = silence. Test #4 is BLOCKING. **Files touched:** `crates/audio-capture/src/multi_output.rs` (NEW), `crates/audio-capture/src/lib.rs` (`pub mod multi_output;`), no new crate deps (`coreaudio-rs` from T-W.2 already covers CFDictionary). **Diff size:** ~300–500 LOC.
...
- [x] **T-W.8** — _Landed: `docs/SETUP.md` ~340 lines / ~3 500 words; 9 sections (§1 Why, §2 Auto-install walkthrough, §3 Manual install brew + .pkg, §4 Manual Multi-Output in Audio MIDI Setup, §5 Verifying audio routing, §6 Troubleshooting × 6 failure modes mapping to R-W.1..R-W.6 + spike-memo Risks #1/#3, §7 FAQ, §8 References, §9 Screenshot inventory of 18 stubs anchored to wizard `data-testid` IDs); README Quickstart line 28 cross-links `docs/SETUP.md` (brew command bumped to `--cask` form to match wizard's argv); 11/11 ACs met; markdown link audit walked at commit time (4 external URLs + 5 repo paths + 6 in-doc anchors all resolve); screenshot capture deferred to operator-side host per loop CONSTRAINT #3 (placeholder lines are the contract); zero source-tree edits — `bun test` baseline 928/928 from T-W.6 preserved by definition._  End-user docs: `docs/SETUP.md` (NEW) with screenshot stubs (filenames declared; image capture deferred to operator-side host) + step-by-step manual fallback (when wizard fails). Sections: 1) Why BlackHole, 2) Auto-install via wizard, 3) Manual install (Homebrew + .pkg), 4) Manual Multi-Output Device creation in Audio MIDI Setup, 5) Verifying audio routing, 6) Troubleshooting (5 common failures from spike-memo §"Risks #3"). Cross-link from `README.md` "Quickstart" line 27 (currently `brew install blackhole-2ch`) to point at `docs/SETUP.md`. **AC:** doc renders valid markdown (no broken anchors); README updated to link to SETUP; troubleshooting section enumerates ≥ 5 failure modes; manual-fallback steps reproduce the wizard's outcome (verified by hand-trace against T-W.2 + T-W.4 logic). **Dep:** T-W.7 (docs reflect tested behavior, not aspirations). **Risk:** Doc drift if T-W.<N> review docs differ from final landed code — mitigated by writing T-W.8 LATE in the sequence. **Files touched:** `docs/SETUP.md` (NEW), `README.md` (+1 link). **Diff size:** ~250–400 LOC of markdown.

- [x] **T-W.9** — _Landed: 6 e2e cases across 5 ACs (E2E-1 happy path / E2E-2 spy assertions / E2E-3 3/3 deterministic re-runs / E2E-4 brew-not-found manual fallback / E2E-5 verify-silent retry × 3); 6/6 pass at file scope; 934/934 pass at workspace scope (was 928 baseline + 6 new); `bun run typecheck` clean; determinism verified by 3 sequential `bun run test tests/e2e/blackhole-wizard.e2e.test.ts` invocations with identical results; fake timers + drained-queue spy assertions guard against flake; file extension is `.e2e.test.ts` (not `.tsx`) per INDEX-declared filename, JSX swapped for `React.createElement(SetupWizard, {...})` (same render output, no behavioral diff); fixture file types every response against the production interfaces so any shape drift fail-compiles via `bun run typecheck`; T-W.7 not blocking (e2e tests the prop-injected invoker contract that T-W.7 will adapt); T-W.5 not blocking (verify mocks return synthesized `VerifyReport` matching the already-pinned interface)._  Fresh-Mac E2E simulation: `tests/e2e/blackhole-wizard.e2e.test.ts` mocks all 4 Tauri commands (detect / install / configure / verify) + a fixture sequence simulating fresh Mac → installed → configured → verified. Assert wizard reaches "All set" panel after walking the 5 steps with deterministic mock responses; assert 3/3 deterministic runs (no flake budget). Plus 2 error paths: brew-not-found → manual URL link rendered, verify-silent → retry loop bounded to 3 attempts then surfaces error. **AC:** test passes 3/3 consecutive runs; mocks adhere to the Tauri-command response Zod schemas defined in T-W.7; no real CoreAudio / brew calls fire (verified by spy assertions). **Dep:** T-W.7. **Risk:** Wall-clock flake (real timers in retry loop) — mitigated by `vi.useFakeTimers()` per existing Phase 4 e2e pattern. **Files touched:** `tests/e2e/blackhole-wizard.e2e.test.ts` (NEW), `tests/e2e/__fixtures__/wizard-states.ts` (NEW). **Diff size:** ~350–500 LOC.
