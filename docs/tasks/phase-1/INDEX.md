...
- [x] **T-1.13** — E2E wiring + smoke. WAV-fixture fallback for the audio half if mic permission unavailable. **AC:** simulated run end-to-end (fixture → capture → chunker → STT → UI → mark question → mocked Claude stream → answer rendered → persisted → reload). 10-step manual `PHASE-BROWSER-TEST.md` documents the human-verified golden path on hardware. Dep: T-1.1 … T-1.12. **Outcome:** new `crates/helper-daemon::Repo::save_meeting_snapshot` (single-txn helper + 4 cargo tests E1..E4); new `src-tauri::{save_meeting, load_meetings}` Tauri commands + `RepoHandle` managed state + `Builder::setup` opens `~/Library/Application Support/meeting-copilot/meetings.db` (2 cargo tests E5..E6); new `src/persist/persistClient.ts` + `src/store/historyStore.ts` + `src/hooks/useMeetingPersist.ts` + `src/components/PastMeetings.tsx`; full vitest E2E in `src/__tests__/E2E.test.tsx` mocking `@tauri-apps/api/{core,event}` + `@anthropic-ai/sdk` exercises the simulated golden-path. **Subtle bug fixed mid-task:** zustand v5 fires separate React updates per selector subscription, re-firing the save effect 3-4× per `set()`. Fix: subscribe only to `status` and read the rest via `getState()` inside the effect. 14 new vitest (4 persistClient + 5 useMeetingPersist + 2 historyStore + 3 E2E) → cumulative **104** (90 prior + 14); 6 new cargo (4 repo + 2 src-tauri) → cumulative **83** (77 prior + 6, code-complete, hardware verify pending per blocked-action #3). `bun run typecheck` clean.
