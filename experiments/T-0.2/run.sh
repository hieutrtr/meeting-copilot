...
# T-0.2 driver — runs blackhole_capture.swift, captures stdout/stderr to out/run.log,
# pretty-prints metrics, refreshes canonical results.json copy, runs afinfo.
...
BIN="$OUT_DIR/blackhole_capture"
WAV="$OUT_DIR/blackhole.wav"
...
if [ ! -x "$BIN" ] || [ "$SCRIPT_DIR/blackhole_capture.swift" -nt "$BIN" ]; then
  echo "[run] swiftc -O $SCRIPT_DIR/blackhole_capture.swift -o $BIN" | tee -a "$LOG"
  if ! swiftc -O "$SCRIPT_DIR/blackhole_capture.swift" -o "$BIN" 2>&1 | tee -a "$LOG"; then
